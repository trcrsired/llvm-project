...
// RUN: %if webassembly-registered-target %{ %clang_cc1 -triple wasm32-unknown-unknown -std=c++26 -fherbceptions -fcxx-exceptions -fexceptions -exception-model=wasm -emit-llvm -o - %s | FileCheck %s --check-prefix=WASM %}
